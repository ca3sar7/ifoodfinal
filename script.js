...
// js/config.js — Base URL da API do projeto
if (typeof API_BASE === 'undefined') {
...
function initPix() {
    ...
    async function postPixStatusWithSessionRetry() {
    const send = async () => {
        try {
...
function initAdmin() {
    ...
    const loadGatewayOrderDailyStats = async () => {
        ...
        try {
            ...
            const url = new URL(`${API_BASE}/admin/leads`, window.location.origin);
    ...
    const runGatewayTests = async () => {
        ...
        try {
            const res = await adminFetch(`${API_BASE}/admin/gateway-test-pix`, {
                method: 'POST',
                body: JSON.stringify({
    ...
    const loadIpBlacklist = async () => {
        if (!ipBlacklistBody) return;
        const res = await adminFetch(`${API_BASE}/admin/ip-blacklist`);
        const data = await res.json().catch(() => ({}));
        if (!res.ok) {
    ...
    const consultLeadTransaction = async () => {
        ...
        const res = await adminFetch(`${API_BASE}/admin/pix-reconcile`, {
            method: 'POST',
            body: JSON.stringify({
                txid,
                gateway,
    ...
    const checkAuth = async () => {
        const res = await adminFetch(`${API_BASE}/admin/me`);
        return res.ok;
    };
    ...
    const runUtmfyTest = async () => {
        ...
        const res = await adminFetch(`${API_BASE}/admin/utmfy-test`, { method: 'POST' });
        if (!res.ok) {
            const detail = await res.json().catch(() => ({}));
            const reason =
    ...
    const runUtmfySale = async () => {
        ...
        const res = await adminFetch(`${API_BASE}/admin/utmfy-sale`, { method: 'POST' });
        if (!res.ok) {
            const detail = await res.json().catch(() => ({}));
            const reason =
                detail?.detail?.reason ||
    ...
    const runPushcutTest = async () => {
        ...
        const res = await adminFetch(`${API_BASE}/admin/pushcut-test`, { method: 'POST' });
        const data = await res.json().catch(() => ({}));
    ...
    const runDispatchProcess = async () => {
        ...
        const res = await adminFetch(`${API_BASE}/admin/dispatch-process`, { method: 'POST' });
        const data = await res.json().catch(() => ({}));
        if (!res.ok || !data?.ok) {
    ...
    const loadCloners = async () => {
        ...
        try {
            const res = await adminFetch(`${API_BASE}/admin/clonadores`);
            const data = await res.json().catch(() => ({}));
            renderCloners(data);
            if (clonersStatus) {
    ...
    const exportLeads = async () => {
        ...
        try {
            const url = new URL(`${API_BASE}/admin/leads/export`, window.location.origin);
    ...
    const loadPageCounts = async () => {
        if (!pagesGrid) return;
        const res = await adminFetch(`${API_BASE}/admin/pages`);
        if (!res.ok) return;
        const data = await res.json();
    ...
    const loadBackredirects = async () => {
        ...
        const res = await adminFetch(`${API_BASE}/admin/backredirects`);
        if (!res.ok) return;
    ...
    const loadSalesInsights = async ({ manual = false } = {}) => {
        ...
        const url = new URL(`${API_BASE}/admin/sales-insights`, window.location.origin);
        const activeRange = getActiveOverviewRange();
    ...
    const loadGatewaySales = async ({ keepSelection = false } = {}) => {
        ...
        const url = new URL(`${API_BASE}/admin/gateway-sales`, window.location.origin);
        const activeRange = getActiveOverviewRange();
    ...
    loginBtn?.addEventListener('click', async () => {
        ...
        const res = await adminFetch(`${API_BASE}/admin/login`, {
            method: 'POST',
...
async function ensureApiSession(force = false) {
    ...
    state.apiSessionPromise = fetch(`${API_BASE}/site/session`, {
        method: 'GET',
        cache: 'no-store',
        credentials: 'same-origin'
    })
...
function trackPageView(page) {
    ...
    const send = () => fetch(`${API_BASE}/lead/pageview`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'same-origin',
        body: JSON.stringify({
...
function trackLead(eventName, extra = {}) {
    ...
    try {
        ...
        if (navigator.sendBeacon) {
            ...
            navigator.sendBeacon(`${API_BASE}/lead/track`, blob);
            return;
        }
        fetch(`${API_BASE}/lead/track`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body,
            keepalive: true
